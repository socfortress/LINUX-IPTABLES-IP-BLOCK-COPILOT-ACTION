...
if iptables -C INPUT -s "$IP" -j DROP 2>/dev/null; then
  STATUS="already_blocked"; REASON="Rule already present"
else
  if iptables -w -I INPUT -s "$IP" -j DROP 2>/dev/null; then
    STATUS="blocked"; REASON="Added DROP rule to INPUT"
  else
    if iptables -C INPUT -s "$IP" -j DROP 2>/dev/null; then
      STATUS="already_blocked"; REASON="Rule appeared concurrently"
    else
      STATUS="failed"; REASON="iptables command failed (permissions/lock?)"
    fi
  fi
fi
